...
if __name__=='__main__':
    ...
    parser.add_argument("--ycb_model_path", type=str, default="/home/cvipl-ubuntu/Workspace/datasets/6DoF/ho3d/YCB_Video_Models", 
    ...
    pred_pose = est.register_any6d(
                                K=intrinsic, # 카메라 정보 (눈)
                                rgb=color,   # RGB 이미지 (시각)
                                depth=depth, # 깊은 정모 (거리)
                                ob_mask=mask,# 물체 마스크 (관심 영역) 
                                iteration=5, # ICP 반복 횟수
                                name=f'demo'
                                )
