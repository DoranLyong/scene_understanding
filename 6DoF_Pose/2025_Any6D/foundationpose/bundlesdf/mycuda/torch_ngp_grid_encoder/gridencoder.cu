/*
 * [PyTorch 2.x 호환성]
 * AT_DISPATCH_FLOATING_TYPES 매크로에서 .type() → .scalar_type() 로 변경 필요
 * 이 파일은 이미 .scalar_type()을 사용하므로 수정 불필요
 */
...
__global__ void kernel_grid(
    ...
    for (uint32_t idx = 0; idx < (1 << D); idx++) {
        ...
        uint32_t index = get_grid_index<D, C>(gridtype, align_corners, 0, hashmap_size, resolution, pos_grid_local);      // Hashing
